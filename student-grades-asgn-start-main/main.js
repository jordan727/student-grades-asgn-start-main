...
function randomTo100() {
  ...
  let randGrade = Math.floor(Math.random() * grades.length);
  grades.splice(randGrade - 1, 1, 100);
}

function addRandomGrade() {
  // Add a random grade between 0 and 100 to the end of the array.
  outputEl.innerHTML = "Add random grade";
  let addRandGrade = Math.floor(Math.random() * maxGrade);
  grades.push(addRandGrade);
}
...
function countBelow50() {
  ...
  alert(belowFifty + " Grades below 50.")
}
...
function increaseGradesBy10() {
  ...
  for (let n = 0; n < grades.length; n++) {
    grades.splice(n, 1, Math.floor(grades[n] += 0.1 * grades[n]));
    console.log(grades[n] + " this is grade " + n);
    if (grades[n] > 100) {
      grades[n] = maxGrade;
    }
  }
